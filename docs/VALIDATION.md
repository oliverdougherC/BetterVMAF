...
The combined local native suite passes46 tests. Release build and static analysis pass; the package verifies exact embedded source revision, executable hash, sandbox entitlements, models and mounted native execution. Final-head macOS CI and artifact hashes are linked on PR#3 and in its handoff. Local XCTest UI-runner startup and Instruments failures are recorded above; no skipped or failed check is represented as a pass. The PR remains draft while required profiling/environment acceptance gaps remain open.
