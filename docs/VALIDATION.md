...
- 20 focused core/admission tests pass, including actual kill/reap and both-pipe drains, concurrency ownership, parser boundaries, metadata/time/geometry/refusals, source changes and output budgets.
...
- Mounted Debug package checks passed, including ad-hoc signatures, native metric execution with system-only runtime paths, resources, notices and corresponding source archive. A sandbox-signed Release DMG subsequently passed actual input selection, analysis, frame navigation, metric switching, JSON/PDF save and live cancellation/restart; [packaged interactive evidence](evidence/native-packaged-interactive.json).
...
The combined local native suite passes46 tests; macOS15 CI also passed those46 tests and both native UI smoke cases. Release build and static analysis pass; the package verifies exact embedded source revision, executable hash, sandbox entitlements, models and mounted native execution. Exact-head CI and review-artifact hashes are linked on PR#3 and in its handoff. The hosted package job validates the full DMG on a fresh Mac; it never creates a public release. Local XCTest UI-runner startup and Instruments failures are recorded above; no skipped or failed check is represented as a pass. The PR remains draft while required profiling/environment acceptance gaps remain open.

The committed packaged-interaction checkpoint records the app revision actually inspected. The final DMG's adjacent `.manifest.json` records its exact current source revision, executable and installer hashes, and verified sandbox entitlements. Generated installer bytes may change on repackaging. Main now requires the stable macOS build/test/package check; archive and release tags remain untouched.
